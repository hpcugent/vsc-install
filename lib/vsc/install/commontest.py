...
def run_ruff(cmd):
    """Run ruff and return the failures"""
    completed = subprocess_run(" ".join(cmd), shell=True, stdout=PIPE, stderr=PIPE, check=True)
    completed.check_returncode()
    return completed

def run_ruff_format_check(base_dir):
    """Run ruff format check and return the failures"""
    log.info(f"Running ruff format check on {base_dir}")
    cmd = ["ruff", "format", "--check", base_dir]
    return run_ruff(cmd)

def run_ruff_check(base_dir):
    """Run ruff check and return the failures"""
    log.info(f"Running ruff check on {base_dir}")
    cmd = ["ruff", "check", base_dir]
    run_ruff(cmd)
...
class CommonTest(TestCase):
    ...
    def test_ruff_check(self):
        """Test if the code is ok according to the ruff rules."""
        vsc_ci_cfg = parse_vsc_ci_cfg()
        self.assertTrue(vsc_ci_cfg[RUN_RUFF_CHECK])
        try:
            completed = run_ruff_check(self.setup.REPO_BASE_DIR)
            self.assertEqual(completed.returncode, 0)
            self.assertEqual(completed.stdout, "")
            self.assertEqual(completed.stderr, "")
        except CalledProcessError as err:
            self.assertTrue(err.returncode == 0, f"ruff format failures: {pprint.pformat(err.stdout, width=500)}")

    def test_ruff_format_check(self):
        """Test if the formatting is ok according to the ruff rules."""
        vsc_ci_cfg = parse_vsc_ci_cfg()
        self.assertTrue(vsc_ci_cfg[RUN_RUFF_FORMAT_CHECK])
        try:
            completed = run_ruff_format_check(self.setup.REPO_BASE_DIR)
            self.assertEqual(completed.returncode, 0)
            self.assertEqual(completed.stdout, "")
            self.assertEqual(completed.stderr, "")
        except CalledProcessError as err:
            failure = err.returncode == 0
            self.assertTrue(failure, f"ruff format failures: {pprint.pformat(err.stdout, width=500)}")
