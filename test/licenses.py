...
class LicenseTest(TestCase):
    ...
    def test_known_licenses(self):
        ...
        self.assertEqual(total_licenses, 3, msg=f"shared_setup has {total_licenses} licenses")
        ...
        for short, data in KNOWN_LICENSES.items():
            ...
            self.assertTrue(os.path.isfile(fn), msg=f"license {short} is in known_licenses directory")
            ...
            self.assertFalse(md5sum in md5sums, msg=f"md5sum for license {md5sum} is unique")

            lic_name, classifier = self.setup.get_license(license_name=fn)
            self.assertEqual(lic_name, os.path.basename(fn), msg=f"file {fn} is license {lic_name}")
            self.assertTrue(
    ...
    def test_release_on_pypi(self):
        ...
        for short in KNOWN_LICENSES:
            self.assertEqual(
                self.setup.release_on_pypi(short), short in PYPI_LICENSES, msg=f"can {short} be usd to release on pypi"
            )
