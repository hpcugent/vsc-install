...
class TestSetup(TestCase):
    ...
    def test_get_name_url(self):
        ...
        for fn in [
            ...
            self.assertEqual(
                self.setup.get_name_url(os.path.join(self.setup.REPO_TEST_DIR, "setup", fn), version="0.1.2"),
                res,
        ...
        self.assertEqual(
            self.setup.get_name_url(
                os.path.join(self.setup.REPO_TEST_DIR, "setup", fn), version="0.1.2", license_name="LGPLv2+"
            ),
            res,
        ...
        self.assertEqual(
            self.setup.get_name_url(os.path.join(self.setup.REPO_TEST_DIR, "setup", fn), version="0.1.0"),
            res_brussel,
            msg=f"determined name and url from {fn} file with license",
        )
    ...
    def test_sanitize(self):
        ...
        self.assertEqual(
            self.setup.sanitize("anything >= 5"), "anything >= 5", msg="no prefixing with VSC_RPM_PYTHON not set"
        )
        self.assertEqual(
            self.setup.sanitize("vsc-xyz >= 10"),
            "vsc-xyz >= 10",
        ...
        self.assertEqual(
            self.setup.sanitize("pycrypto == 12"),
            "pycrypto == 12",
            msg="packages in PYTHON_BDIST_RPM_PREFIX_MAP are not repalced with value with VSC_RPM_PYTHONnot set",
        )
        self.assertEqual(
            self.setup.sanitize("pbs_python <= 13"),
            "pbs_python <= 13",
    ...
    def test_action_target(self):
        ...
        self.mock_stdout(True)
        action_target(
            {"name": "vsc-test", "version": "1.0.0"}, setupfn=fake_setup, urltemplate="http://example.com/%(name)s"
        )
        txt = self.get_stdout()
        self.mock_stdout(False)
        self.assertTrue(re.search(r"^args:\s*\(\)", txt, re.M))
    ...
    def test_parse_target_dependencies(self):
        ...
        package = {
            ...
            "excluded_pkgs_rpm": [],
            "version": "1.0",
            "install_requires": ["vsc-config >= 2.0.0", "vsc-accountpage-clients", "vsc-base > 1.0.0"],
        }
        setup = vsc_setup()
    ...
    def test_parse_remove_dataclasses(self):
        ...
        package = {
            ...
            "excluded_pkgs_rpm": [],
            "version": "1.0",
            "install_requires": ["vsc-config >= 2.0.0", "vsc-accountpage-clients", "vsc-base > 1.0.0", "dataclasses"],
        }
        setup = vsc_setup()
        setup.private_repo = True
        new_target = setup.parse_target(package)
